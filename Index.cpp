...
std::mutex Index::mutexIndexMap;

void Index::updateDocumentBase(const std::vector<std::string>& input_docs)
{
    if (input_docs.empty())
    {
...
std::vector<Entry> Index::getWordCount(const std::string& word)
{
...
void Index::indexTheFile(const std::string& fileContent, size_t docId)
{
    // Split doc on words.
...
size_t Index::getWordCountInDoc(const std::string& word, const size_t doc_id) const
{
    if (indexingIsOngoing) {
        std::cout << "Index is ongoing, please repeat the request later.\n";
        return 0;
